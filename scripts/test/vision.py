...
# 显示指定类别的 CIFAR-100 图片
def show_cifar100_image_by_class(data_dir, class_label):
    # 加载数据集
    data = load_cifar100_data(data_dir)
    images, labels = data['data'], data['fine_labels']
    
    # CIFAR-100 数据集中的图片是按照列来排列的 (32*32*3, )
    # 因此我们需要重新排列成 32x32x3 的格式
    ...
    print(f"No images found for class label {class_label}")

# CIFAR-100 数据集的根目录
data_dir = '/home/corleone/ROS1/realsense/src/data/cifar-100-python'  # 请替换为你的数据集路径

# 要显示的类别标签
class_label = int(input("Enter the class label (0-99): "))

# 显示指定类别的 CIFAR-100 图片
show_cifar100_image_by_class(data_dir, class_label)